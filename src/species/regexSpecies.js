    ...
    await lines.forEach(line => {
        ...
        if(startFound){
            ...
            if(matchMove){
                ...
                const rawTMHM = fetch(`https://raw.githubusercontent.com/degland/Radical-Red-Pokedex/main/data/species/tm_compatibility/${count} - ${move}.txt`)
                .then(promises => {
        ...
        if(startFound){
            ...
            if(matchMove){
                ...
                if(filterUnusedTutor.includes(move)){
                    const rawTutor = fetch(`https://raw.githubusercontent.com/degland/Radical-Red-Pokedex/main/data/species/tutor_compatibility/${count} - ${move}.txt`)
                    .then(promises => {
...
async function regexSprite(textSprite, species){
    ...
    await lines.forEach(line => {
        ...
        if(matchSpecies){
            ...
            if(matchURL){
                let url = `https://raw.githubusercontent.com/degland/Radical-Red-Pokedex/main/data/species/frontspr/${matchURL[0].replace("Tiles", ".png")}`
